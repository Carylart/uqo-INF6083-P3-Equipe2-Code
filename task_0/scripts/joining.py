...
# Ensure the repository root is on sys.path so we can import path.py
...
def get_manifest(
    ...
    base = path.OUTPUTS / "processed"
    raw_base = path.OUTPUTS / "raw" / "parquet"
...
def save_diagnostics(
    result: Dict[str, Any], 
    out_dir: str = str(path.JOINING / "diagnostics")
) -> Dict[str, str]:
    out = Path(out_dir)
...
def run_all(
    ...
    # Collecte d'abord les parent_asin des interactions disponibles
    needed_asins = set()
    for name, cfg in manifest.items():
        ...
        if not path_status.get(name, False):
            continue
        pf = pq.ParquetFile(cfg["paths"][0])
        for batch in pf.iter_batches(batch_size=500_000, columns=["parent_asin"]):
            needed_asins.update(batch.column("parent_asin").to_pylist())

    if verbose:
        print(f"\n  parent_asin uniques dans interactions : {len(needed_asins):,}")

    # Charge la metadata en streaming, ne garder que les lignes utiles
    meta_cols = ["parent_asin"] + METADATA_TEXT_COLS + METADATA_STRUCT_COLS
    meta_chunks = []
    pf_meta = pq.ParquetFile(meta_cfg["paths"][0])
    needed_arr = pa.array(list(needed_asins))

    for batch in pf_meta.iter_batches(batch_size=500_000, columns=meta_cols):
        table = pa.Table.from_batches([batch])
        mask = pc.is_in(table.column("parent_asin"), value_set=needed_arr)
        filtered = table.filter(mask)
        if filtered.num_rows > 0:
            meta_chunks.append(filtered.to_pandas())

    meta_df = pd.concat(meta_chunks, ignore_index=True) if meta_chunks else pd.DataFrame(columns=meta_cols)
    del meta_chunks, needed_arr
    ...
    if verbose:
        print(f"  metadata filtrée : {len(meta_df):,} lignes (vs 4.4M total)")
    ...
    for name, cfg in manifest.items():
        ...
        if materialize_joined:
            out_path = save_joined_dataset(joined_df, name=name + "_clean", out_dir=str(path.JOINING), verbose=verbose)
            out_path = save_joined_dataset(joined_df, name=name + "_clean", out_dir=str(path.JOINING), verbose=verbose)
        final_datasets[name] = {
            "path": out_path,
    ...
    if export_artifacts:
        result["artifacts"] = save_diagnostics(result, out_dir=str(path.JOINING / "diagnostics"))
...
def cli_print_md_results(verbose: bool = True) -> None:
    if verbose:
        print(Path(path.JOINING / "diagnostics" / "joining_diagnostics.md").read_text(encoding="utf-8"))
