...
# Ensure the repo root is on sys.path so we can import path.py
...
# NOTE: la structure actuelle place les jsonl dans `data/input/raw/jsonl` et
# les fichiers parquet générés dans `data/outputs/raw/parquet`.
RAW_JSONL_DIR = path.INPUT / "raw" / "jsonl"
RAW_PARQUET_DIR = path.OUTPUTS / "raw" / "parquet"
PROCESSED_DATA_DIR = path.OUTPUTS / "processed"

RAW_JSONL_GLOB = str(RAW_JSONL_DIR / "*.jsonl")
RAW_PARQUET_GLOB = str(RAW_PARQUET_DIR / "*.parquet")
RAW_BOOKS_PATH = str(RAW_PARQUET_DIR / "Books.parquet")
RAW_META_PATH = str(RAW_PARQUET_DIR / "meta_Books.parquet")
...
def jsonl_to_parquet_conversion() -> bool:
    """
    Convertit tous les fichiers JSONL bruts du dossier RAW_JSONL_DIR
    vers des fichiers Parquet dans RAW_PARQUET_DIR.

    La conversion :
    - vérifie si le fichier Parquet existe déjà et est cohérent,
    - reconvertit si le nombre de lignes ou le schéma diffèrent,
    - force certaines colonnes numériques candidates en Float64,
    - effectue une validation finale sur un échantillon.
    """
    ...
    print("+======================================================================+")
    print(f"  Répertoire JSONL source      : {RAW_JSONL_DIR}")
    print(f"  Répertoire Parquet cible     : {RAW_PARQUET_DIR}")
    ...
    for jsonl_path in RAW_JSONL_PATHS:
        ...
        # Construit proprement le chemin cible parquet
        parquet_path = RAW_PARQUET_DIR / f"{jsonl_path.stem}.parquet"
        ...
        if parquet_path.exists():
            try:
                n_parquet = pl.scan_parquet(str(parquet_path)).select(pl.len()).collect().item()
                cols_parquet = set(pl.scan_parquet(str(parquet_path)).collect_schema().names())
                ...
                if n_parquet != n_jsonl:
                    ...
                    schema_jsonl = pl.scan_ndjson(str(jsonl_path), infer_schema_length=1).collect_schema()
                ...
                print(f"   Fichier Parquet invalide ({e}), reconversion : {parquet_path}")
                if parquet_path.exists():
                    parquet_path.unlink()

        else:
            print(f"   Fichier Parquet absent, conversion : {parquet_path}")

        if not needs_reconversion:
            continue

        # -- 2. Détection des colonnes à surcharger ---------------------------
        schema_initial = pl.scan_ndjson(str(jsonl_path), infer_schema_length=1).collect_schema()
        ...
        lf = pl.scan_ndjson(str(jsonl_path), **kwargs)
        ...
        lf.sink_parquet(str(parquet_path))
        del lf
        gc.collect()
        ...
        cols_jsonl = set(pl.scan_ndjson(str(jsonl_path), infer_schema_length=1).collect_schema().names())
        cols_parquet = set(pl.scan_parquet(str(parquet_path)).collect_schema().names())
        ...
        if n_sample == 0:
            print("  ✓ Vérification ignorée (fichier vide)")
        else:
            read_kwargs = {"schema_overrides": schema_overrides} if schema_overrides else {}

            df_jsonl_sample = pl.read_ndjson(str(jsonl_path), n_rows=n_sample, **read_kwargs)
            ...
            df_parquet_sample = pl.read_parquet(str(parquet_path), n_rows=n_sample)
